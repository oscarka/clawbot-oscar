...
export async function runMessageAction(
  ...
  // Feishu: when agent passes user ID (ou_xxx) but context is group (oc_xxx), use group
  // so replies go to the group chat instead of failing with "invalid receive_id" on DM
  const channelHint = typeof params.channel === "string" ? params.channel.trim() : "";
  const feishuChannel =
    channelHint.toLowerCase() === "feishu" ||
    input.toolContext?.currentChannelProvider?.toLowerCase() === "feishu";
  if (feishuChannel && actionRequiresTarget(action)) {
